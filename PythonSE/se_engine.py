"""se_engine.py — symbolic execution for mini IMP.
...
from z3 import (
    And, ExprRef, BitVec, BitVecVal, Bool, BoolVal, LShR, Not, Or, Solver, sat, unsat,
    BoolSort, BitVecSort, IntSort,
)
...
def eval_arg(arg: ast.arg):
    print(arg)
    match arg:
        case ast.arg(arg=name, annotation=ast.Name(id="bool")):
            return Bool(name)

        case ast.arg(arg=name, annotation=ast.Name(id="int")):
            print(name)
            return BitVec(name, BW)

        case ast.arg(arg=name, annotation=None):
            print(name)
            return BitVec(name, BW)

        case ast.arg(arg=name, annotation=other):
            raise NotImplementedError(
                f"Unsupported annotation for {name}: {ast.dump(other)}"
            )


def eval_args(args: list[ast.arg]):
    return [eval_arg(arg) for arg in args]


# --- Get Concrete Values --------------------------------------------------

def evaluate_z3_expr(model, z3_arg: ExprRef):
    sort = z3_arg.sort()
    value = model.evaluate(z3_arg, model_completion=True)

    if sort == BoolSort():
        return bool(value)
    elif sort.kind() == BitVecSort(BW).kind():
        return value.as_signed_long()
    elif sort == IntSort():
        return value.as_long()

    raise NotImplementedError(f"Unsupported Z3 sort: {sort}")
...
def symbolic_execute(fn, *, max_depth=20, mode='dsa', bitwidth=32):
    ...
    with _bitwidth(bitwidth):
        ...
        p = Path()
        fn_args = eval_args(tree.args.args)
        for arg in fn_args:
            p.state[arg.decl().name()] = arg
        return execute_stmts(tree.body, p, max_depth, mode), fn_args


def find_cases_with_solver(pcs, rets, fn_args, extra_constraints):
    s = Solver()
    s.add(*pcs)
    ...
    for arg in fn_args:
        test_case_inputs[arg.decl().name()] = evaluate_z3_expr(model, arg)

    ret_val = None
    if rets is not None:
        ret_val = evaluate_z3_expr(model, rets)

    return test_case_inputs, ret_val


def auto_generate_tests(fn, *, max_depth=20, mode='dsa', timeout_ms=5000, bitwidth=32, extra_constraints=None):
    ...
    """
    paths, fn_args = symbolic_execute(fn, max_depth=max_depth, mode=mode, bitwidth=bitwidth)

    tests = []
    for i, p in enumerate(paths):
        test_case_inputs, test_case_returns = find_cases_with_solver(p.constraints(), p.return_val, fn_args, extra_constraints)
        tests.append({"inputs": test_case_inputs,
                      "pc": p.constraints(),
                      "symbolic return": p.return_val,
                      "real return": test_case_returns,
                     })
